 ...
 */
/* Add a second input for constant electric field
 * modify by Zigang Song <songzigang@stu.pku.edu.cn>
 * mdp option:
...
public:
    ...
    {
        builder->addUniformArray<real>(name, { a_, omega_, t0_, sigma_ });
    }
...
class ConstantElectricFieldDimension : public ElectricFieldDimension {
public:
    /*! \brief
     * Adds an option section to specify parameters for this field component.
     */
    ...
    {
        auto section = options->addSection(OptionSection(sectionName));
        section.addOption(RealOption("E").store(&a_));
    }
    /*! \brief
     * Creates mdp parameters for this field component.
     */
    void buildMdpOutput(KeyValueTreeObjectBuilder* builder, const std::string& name) const
    {
        builder->addUniformArray<real>(name, { a_ });
    }
...
private:
    ...
    ConstantElectricFieldDimension constant_efield_[DIM];
...
void convertParameters(gmx::KeyValueTreeObjectBuilder* builder, const std::string& value)
{
    ...
    if (sxt.size() == 1) {
        ...
        builder->addValue<real>("E", fromString<real>(sxt[0]));
    } 
    else if (sxt.size() == 4) {
        ...
        GMX_THROW(InvalidInputError("Please specify E0 omega t0 sigma for electric fields or E for constant electric fields"));
    }
...
void ElectricField::initMdpTransform(IKeyValueTreeTransformRules* rules)
{
    ...
    // add for constant electric field
    rules->addRule().from<std::string>("/constant-electric-field-x").toObject("/constant-electric-field/x").transformWith(&convertParameters);
    rules->addRule().from<std::string>("/constant-electric-field-y").toObject("/constant-electric-field/y").transformWith(&convertParameters);
    rules->addRule().from<std::string>("/constant-electric-field-z").toObject("/constant-electric-field/z").transformWith(&convertParameters);
}
...
void ElectricField::initMdpOptions(IOptionsContainerWithSections* options)
{
    ...
    // add for constant electric field
    section = options->addSection(OptionSection("constant-electric-field"));
    constant_efield_[XX].initMdpOptions(&section, "x");
    constant_efield_[YY].initMdpOptions(&section, "y");
    constant_efield_[ZZ].initMdpOptions(&section, "z");
}
...
; format for constant-electric-field-x, etc. is one real variable E (V/nm))";
    builder->addValue<std::string>("comment-electric-field", comment);
    efield_[XX].buildMdpOutput(builder, "electric-field-x");
    efield_[YY].buildMdpOutput(builder, "electric-field-y");
    efield_[ZZ].buildMdpOutput(builder, "electric-field-z");
    // add for constant electric fiield
    constant_efield_[XX].buildMdpOutput(builder, "constant-electric-field-x");
    constant_efield_[YY].buildMdpOutput(builder, "constant-electric-field-y");
    constant_efield_[ZZ].buildMdpOutput(builder, "constant-electric-field-z");
}
...
{
    return efield_[dim].evaluate(t) + constant_efield_[dim].evaluate(t);
}

bool ElectricField::isActive() const
{
    return (efield_[XX].a() != 0 || efield_[YY].a() != 0 || efield_[ZZ].a() != 0 || 
            constant_efield_[XX].a() != 0 || constant_efield_[YY].a() != 0 || constant_efield_[ZZ].a() != 0);
}
